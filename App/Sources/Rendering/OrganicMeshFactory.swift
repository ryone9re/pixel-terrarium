...
enum OrganicMeshFactory {
    ...
    static func fernFrond(leafletPairs: Int, variant: Int) -> MeshResource {
        let pairs = max(3, min(9, leafletPairs))
        ...
        let bend = 0.28 + Float(variant % 3) * 0.075
        let sway = Float(variant % 3 - 1) * 0.045
        let points: [SIMD3<Float>] = (0...9).map { index in
            let progress = Float(index) / 9
            return SIMD3<Float>(
        ...
        builder.appendTube(
            points: points,
            radii: (0...9).map { 0.014 - Float($0) * 0.0009 },
            segments: 5
        )
        ...
        for level in 1...pairs {
            ...
            let length = 0.07 + sin(progress * .pi) * 0.18
            let width = length * 0.11
            builder.appendLeaflet(at: center, sign: -1, length: length, width: width, lift: 0.19)
            builder.appendLeaflet(at: center, sign: 1, length: length, width: width, lift: 0.19)
        }
        builder.appendLeaflet(
            at: points[points.count - 2],
            sign: 1,
            length: 0.15,
            width: 0.032,
            lift: 0.72
    ...
    static func terrainHeight(xPosition: Float, zPosition: Float) -> Float {
        let radial = min(1, sqrt(
            (xPosition / 1.24) * (xPosition / 1.24) +
                (zPosition / 1.09) * (zPosition / 1.09)
        ))
